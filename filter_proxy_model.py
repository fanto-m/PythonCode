#filter_proxy_model.qml
from PySide6.QtCore import QSortFilterProxyModel, Slot, QSettings, Property
from items_model import ItemsModel  # Импортируем ItemsModel для доступа к ролям

class FilterProxyModel(QSortFilterProxyModel):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._filter_field = "name"  # Поле по умолчанию для фильтрации
        self._filter_string = ""
    ...
    filterString = Property(str, get_filter_string, notify=None)  # Можно добавить сигнал notify позже
    filterField = Property(str, get_filter_field, notify=None)

    def _loadSettings(self):
        """Загружает сохраненные настройки фильтра"""
    ...
    def _saveSettings(self):
        """Сохраняет текущие настройки фильтра."""
    ...
    def updateItem(self, row, article, name, description, image_path, category, price, stock, status, unit, manufacturer, barcode):
        print("DEBUG: FilterProxyModel.updateItem called, redirecting to sourceModel")
        try:
            price = float(price) if price is not None and str(price).strip() else 0.0
            stock = int(stock) if stock is not None and str(stock).strip() else 0
            self.sourceModel().updateItem(row, article, name, description, image_path, category, price, stock, status, unit, manufacturer, barcode)
    ...
    def deleteItem(self, row):
        print("DEBUG: FilterProxyModel.deleteItem called, redirecting to sourceModel")
        try:
            self.sourceModel().deleteItem(row)
